<template>
	<div
		v-if="data"
		:class="['wow', 'bounceInUp', 'news__item', `news__item--${data.reversing}`, ((!isMobile) ? 'news__item--hover' : 'news__item--swipe')]"
	>
		...
		<div class="news__content">
			<div class="swipe">
				<div class="swipe-panel">
					...
					<div
						:style="{
							background: `-webkit-gradient(linear, left top, right top, from(transparent), to(${ convertHex(data.color) }))`,
							background: `linear-gradient(to right, transparent 0%, ${ convertHex(data.color) } 100%)`
...
export default {
	...
	mounted() {
		if (process.browser) {
			...
			if (this.isMobile) {
				this.slider = SwipeSlider(this.$el.querySelector('.swipe'), {
					infinite: false,
					slidesToScroll: 2,
					dots: true
				})
