...
function slug(string) {
	...
	const lengths = [];
	for (const key in multicharmap) {
		if (!multicharmap.hasOwnProperty(key)) continue;
	...
	for (let char, i = 0, l = string.length; i < l; i++) {
		...
		if (
			...
			if (symbols && (unicode = symbols(code))) {
		...
		// char = char.replace(/[^.]/g, ' '); // allowed
		char = char.replace(/[^\w\s\-]/g, ""); // allowed
		result += char;
	}

	result = result.replace(/^\s+|\s+$/g, ""); // trim leading/trailing spaces
	result = result.replace(/[-\s]+/g, "-"); // convert spaces
	result = result.replace("-" + "$", ""); // remove trailing separator
