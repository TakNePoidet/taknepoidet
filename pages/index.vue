	...
	<main>
		...
		<div class="section-about" id="about">
			<h2 class="section-title">
				{{$store.state.lang.header.about_me}}
			</h2>
			<div>
		...
		<div class="section-social-networks social_networks" id="social">
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1501 32.167" class="section-social-networks__top">
				<path d="M835,117.166c-330.106,0-615.469-7.71-751-18.9V85H1585V98.348C1449.065,109.494,1164.29,117.166,835,117.166Z" transform="translate(-84 -85)"/>
			</svg>
			<h2 class="section-title">
				{{$store.state.lang.section.social.title}}
			</h2>
			<ul class="social_networks__links">
				<li 
					...
					><a v-bind:href="item.link" target="_blank">
						<div class="social_networks__item__logo"><i v-bind:class="item.icon"></i></div>
						<div class="social_networks__item__nik">{{ item.nik }}</div>
						<div class="social_networks__item__name">{{ item.name }}</div>
					</a>
				</li>
			</ul>
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1501 32.167" class="section-social-networks__bottom">
				<path d="M835,85c-330.107,0-615.469,7.71-751,18.9v13.266H1585V103.818C1449.065,92.672,1164.29,85,835,85Z" transform="translate(-84 -85)"/>
			</svg>
		</div>
		<div class="section-news" id="news">
			...
			<div class="news" v-if="news.length > 1">
				<div v-for="item in news" :key="item.id" v-bind:class="['wow', 'bounceInUp', 'news__item']">
					<a target="_blank" class="news__link" v-bind:href="`https://vk.com/wall20513451_${item.id}`" rel="nofollow">
						...
						<div class="news__content">
							<div class="news__data">{{item.text}}</div>
						</div>
					</a>
				</div>
			</div>
...
export default {
	...
	methods : {
		windowScroll() {
			...
			box.querySelector('.section-background-images__content').style.opacity = (scrollTop === 0) ? 1 : 1/(scrollTop/100*4);
